...
- **Storage-Fehler behoben**: `chrome.storage.session` ist in Content Scripts ohne `setAccessLevel()` nicht verfügbar. Background-Script setzt jetzt korrekt `TRUSTED_AND_UNTRUSTED_CONTEXTS`, Anonymizer nutzt robuste Fallback-Logik mit Probe-Zugriff.
- **Toggle-Deaktivierung repariert**: Extension liess sich nicht korrekt über den Popup-Toggle deaktivieren:
  - MutationObserver wurde in lokaler Variable statt `this._domObserver` gespeichert (nie disconnected)
  - 3 `setInterval`s (DOM-Poll, File-Poll, Response-Check) wurden bei `stopMonitoring()` nie gecleard
  - Event-Listener (input/keydown/paste) wurden nicht von DOM-Elementen entfernt
  - Mehrfaches `startMonitoring()` erzeugte duplizierte Observer und Intervals
- **Event-Handler-Referenzen gespeichert**: Listener werden jetzt in `monitoredElements`-Map gespeichert und bei `stopMonitoring()` korrekt entfernt

#### Performance

- **DOM-Polling reduziert**: Fallback-Interval von 2s auf 5s erhöht (MutationObserver deckt Normalfall ab)
- **File-Polling reduziert**: Von 2s auf 5s
- **Zentrale Cleanup-Methode**: Neues `_cleanupTimers()` verhindert doppelte Observer/Intervals bei Re-Aktivierung
